...
在[交叉导入的公共API][public-api-for-cross-imports]部分了解更多关于`@x`标记法的信息。
...
如果你使用嵌套路由系统（如[Remix][ext-remix]的路由器），使用小部件层的方式可能与扁平路由系统使用页面层的方式相同 — 创建完整的路由块，包括相关的数据获取、加载状态和错误边界。
